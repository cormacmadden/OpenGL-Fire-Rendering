...
vec3 divScalar(vec3 d, float scalar) {
	return vec3(d.v[0] / scalar, d.v[1] / scalar, d.v[2] / scalar);
}

vec3 mat3::operator* (const vec3& rhs) {
	float x = m[0] * rhs.v[0] + m[4] * rhs.v[1] + m[8] * rhs.v[2]; // 0x + 4y + 8z + 12w
	float y = m[1] * rhs.v[0] + m[5] * rhs.v[1] + m[9] * rhs.v[2]; // 1x + 5y + 9z + 13w
	float z = m[2] * rhs.v[0] + m[6] * rhs.v[1] + m[10] * rhs.v[2]; // 2x + 6y + 10z + 14w
	float w = m[3] * rhs.v[0] + m[7] * rhs.v[1] + m[11] * rhs.v[2]; // 3x + 7y + 11z + 15w
	return vec4(x, y, z, w);
}

/*---------------------------------MATRIX FUNCTIONS-----------------------------------*/
mat3 mat4To3(const mat4& mm) {
	return mat3(
		mm.m[0], mm.m[1], mm.m[2],
		mm.m[3], mm.m[4], mm.m[5],
		mm.m[6], mm.m[7], mm.m[8]
	);
}
...
mat4 initRotateMatrix(float RotateX, float RotateY, float RotateZ) {
	mat4 rx, ry, rz;
	
	float x = RotateX * PI / 180;
	float y = RotateY * PI / 180;
	float z = RotateZ * PI / 180;
