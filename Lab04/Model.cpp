...
void Model::UpdatePos(int x, int y, Camera camera)
{
	int width = 1400;
	int height = 700;
	...
	mat4 output_mat = identity_mat4();
	//gluUnProject((GLdouble) x, (GLdouble)y, (GLdouble)0.0, modelview, persp_proj, view, &posX, &posY, &posZ);
	output_mat.m[12] = x;
	output_mat.m[13] = y;
	location = persp_proj * view * output_mat;
	std::cout << "\n Cube x :" << location.m[12] << " Cube y : " << location.m[13] << " Cube y : " << location.m[14];
}

void Model::ProcessKeyboard(MovementDir direction, float deltaTime)
{
	//std::cout << "\n x" << mat2.m[12] << " y " << mat2.m[13] << " z " << mat2.m[14];
	if (direction == BOX_LEFT) {
		mat4 moveMat = identity_mat4();
		float speed = -0.1;
		moveMat.m[12] = speed;
		location = moveMat * location;
	}
	if (direction == BOX_RIGHT) {
		mat4 moveMat = identity_mat4();
		float speed = 0.1;
		moveMat.m[12] = speed;
		location = moveMat * location;
...
void Model::setLoc(vec3 loc) {
	location.m[12] = loc.v[0];
	location.m[13] = loc.v[1];
	location.m[14] = loc.v[3];
}
...
void Model::RenderModel(mat4 transform, unsigned int shaderID)
{
	...
	for (unsigned int i = 0; i < m_Meshes.size(); i++) {
		...
		*/
		...
		glUniformMatrix4fv(matrix_location, 1, GL_FALSE, location.m);
...
void Model::LoadDiffuseTexture(const string& Dir, const aiMaterial* pMaterial, int index)
{
	...
	if (pMaterial->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
		...
		if (pMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
			...
			m_Materials[index].pDiffuse = Texture(GL_TEXTURE_2D, FullPath.c_str());
...
void Model::LoadSpecularTexture(const string& Dir, const aiMaterial* pMaterial, int index)
{
	...
	if (pMaterial->GetTextureCount(aiTextureType_SHININESS) > 0) {
		...
		if (pMaterial->GetTexture(aiTextureType_SHININESS, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
			...
			m_Materials[index].pSpecularExponent = Texture(GL_TEXTURE_2D, FullPath.c_str());
